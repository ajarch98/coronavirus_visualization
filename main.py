...
for i in range(5, len(data.columns), 2):
    ...
    df = data[['latitude', 'longitude', 'country', 'location', col_names[cases_col], col_names[deaths_col]]]
    ...
    df_cases['text'] = df_cases['country'] + '<br>' + df_cases['location'] + '<br>' + 'confirmed cases: ' + (df_cases[df_cases.columns[-2]].astype(int)).astype(str) + '<br>' +\
        'deaths: ' + (df_cases[df_cases.columns[-1]].astype(int)).astype(str)
    ...
    df_deaths['text'] = df_deaths['country'] + '<br>' + df_deaths['location'] + '<br>' + 'confirmed cases: ' + (df_deaths[df_deaths.columns[-2]].astype(int)).astype(str) + '<br>' +\
        'deaths: ' + (df_deaths[df_deaths.columns[-1]].astype(int)).astype(str)
    ...
    fig.add_trace(go.Scattergeo(name='Deaths', lon=deaths_lng, lat=deaths_lat, visible=False, hovertemplate=df_deaths['text'], text="Text", mode='markers',
                                marker=dict(size=10, opacity=1, color='Red', symbol='circle')))
...
fig.update_layout(sliders=sliders,
                  title='Rise of the Novel Coronavirus<br>nA Python Data Visualization by Advait Joshi', title_x=0.5,
                  legend_title='Key',
                  height=600)
fig.show()
